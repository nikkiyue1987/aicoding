...
import { ChatSession } from '../services/minimaxService';
...
const DictionaryView: React.FC<DictionaryViewProps> = ({ nativeLang, targetLang, onSave, savedIds }) => {
  ...
  const handleSearch = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const textPromise = minimaxService.lookupWord(query, nativeLang.name, targetLang.name);
      const imagePromise = minimaxService.generateImageForWord(query);
      ...
      chatSessionRef.current = minimaxService.createChatSession(query, targetLang.name, nativeLang.name);
  ...
  const handleSpeak = async (text: string, id: string) => {
    if (speaking) return;
    setSpeaking(id);
    await minimaxService.speakText(text);
    setSpeaking(null);
  };
          ...
          <div className="bg-white rounded-3xl shadow-lg overflow-hidden border border-gray-100">
            ...
            <div className="p-6 relative">
              <button
                ...
                className={`absolute top-6 right-6 p-2 rounded-full transition-colors ${savedIds.has(result.word) ? 'bg-yellow-100 text-yellow-600' : 'bg-gray-100 text-gray-400 hover:bg-gray-200'
                  }`}
              >
